            ...
            setTimeout(() => {
                if (document.querySelector('app-equipe-mini-card')) {
                    if (document.querySelector("div[id='naorepete-empenhada']")) {
                        ...
                        var eqps = ['A1', 'C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8', 'C9', 'C10', 'C11', 'C12', 'C13', 'C14', 'C15', 'D1', 'D2', 'D3', 'D4', 'D5', 'D6', 'D7', 'D8', 'C9', 'R1', 'R2', 'R3', 'R4', 'R5', 'P1', 'P2', '21', '22', '31', '32', '41', '42', '51', '52', '61', '62', '71', '72', '81', '82', '91', '92', 'COGM'];
                        ...
                        if (gms) {
                            ...
                            document.getElementById('sel_area_edit_equip').addEventListener('change', function () {
                                ...
                                guardas.forEach(function (item) {
                                    op += '<option value=' + item + '>' + item.replace(/\D/g, "") + '</option>';
                                });
                                if (area != 'COGM') {
                            ...
                            document.getElementById('txt_separador_equipes').addEventListener('input', function () {
                                ...
                                document.querySelector('#div_separador_equipes').querySelector('thead').querySelector('button').addEventListener('click', function () {
                                    document.querySelectorAll('#div_separador_equipes tbody button').forEach(button => {
                                        button.click();
                                    })
                                });
...
async function buscarDadosEquipe(unidadeUuid, equipeUuid) {
    const situacaoUnidadeRequisicao = await fetch(
        `https://cadweb.sinesp.gov.br/cad-equipe-servico/unidade-servico/${unidadeUuid}`,
        {
            credentials: "include"
        }
    );

    if (!situacaoUnidadeRequisicao.ok) {
        throw new Error(`Erro HTTP: ${situacaoUnidadeRequisicao.status}`);
        return;
    }
    const situacaoUnidadeResposta = await situacaoUnidadeRequisicao.json();
    console.log(situacaoUnidadeResposta);
    const situacaoUnidade = situacaoUnidadeResposta.situacao;
    ...
    const dadosEquipe = {};
    dadosEquipe.situacao = situacaoUnidade;
...
async function encerrarServico(uuidUnidadeServico) {
    const encerrarSvRequisicao = await fetch( //atualiza Uuidequipamentos
        `https://cadweb.sinesp.gov.br/cad-equipe-servico/unidade-servico/${uuidUnidadeServico}/encerrar-servico`,
        {
            method: "PUT",
            credentials: "include",
            headers: {
    ...
    if (!encerrarSvRequisicao.ok) {
        throw new Error(`Erro HTTP: ${encerrarSvRequisicao.status}`);
        return false;
    }
    console.log(encerrarSvRequisicao.status);
    return true;
}
...
async function enviarDadosEdicaoEquipe(botao) {
    ...
    const dados = Array.from(botao.closest('tr').querySelectorAll('input,select:not([id=sel_area_edit_equip])')).map(campo => {
        if (campo.id == "sel_equipe_edit_equip") return campo.selectedOptions[0].innerText //no caso de ser o botão da linha dos selects
        return campo.value
    });
    ...
    const dadosDaEquipe = {};
    dadosDaEquipe["nomeEquipe"] = nomeEquipe;
    ...
    const situacaoDaUnidade = await buscarDadosEquipe(dadosDaEquipe.uuidUnidadeServico, dadosDaEquipe.uuidEquipe);
    console.log(situacaoDaUnidade);
    if (!situacaoDaUnidade) {
        console.log('não foi possível verificar situacao da unidade');
        return;
    }
    let unidadeEncerrada = true;
    if (situacaoDaUnidade.situacao != 'AT') {
        unidadeEncerrada = await encerrarServico(dadosDaEquipe.uuidUnidadeServico);
    }
    if (!unidadeEncerrada) {
        console.log('não foi possível encerrar a unidade de serviço');
        return;
    }
    const equipeEditada = await editarEquipe(dadosDaEquipe);
    if (!equipeEditada) {
        console.log('equipe n editada');
        return;
    }
    ...
    if (equipamentos.length > 0) {
        ...
        if (!equipamentosEditados) {
            console.log('equipamentos n editados');
            return;
        }
    }

    const equipeIniciada = await iniciarServico(dadosDaEquipe["uuidUnidadeServico"], dadosDaEquipe["uuidEquipe"]);
    console.log(equipeIniciada)
    if (!equipeIniciada) {
        console.log('equipe n iniciada');
        return;
    }
